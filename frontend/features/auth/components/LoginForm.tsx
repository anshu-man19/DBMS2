...
export function LoginForm() {
  ...
  const onSubmit = async (values: LoginFormValues) => {
    ...
    if (response?.error) {
      setError("password", {
        type: "manual",
        message: "Use the password set during registration",
      });
    }
    enqueueSnackbar(
      "Login failed. Use a registered email and its saved password.",
      { variant: "error" },
    );
  };
        ...
        <Stack component="form" spacing={2.5} onSubmit={handleSubmit(onSubmit)}>
          <div>
            ...
            <Typography color="text.secondary">
              Sign in using the email you registered and the password you
