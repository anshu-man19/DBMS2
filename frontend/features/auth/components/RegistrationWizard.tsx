  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      if (!response.ok || !payload?.ok) {
        enqueueSnackbar(
          payload?.message ?? "Failed to send OTP. Please try again.",
          { variant: "error" },
        );
        return;
      }
  ...
  const submitForm = async (values: RegistrationFormValues) => {
    ...
    try {
      ...
      if (!registerResponse.ok || !registerPayload?.ok) {
        enqueueSnackbar(
          registerPayload?.message ?? "Unable to create recruiter account.",
          { variant: "error" },
        );
        return;
      }
      ...
      enqueueSnackbar("Registration completed, but automatic login failed.", {
        variant: "warning",
      });
    } finally {
      setSubmitting(false);
        ...
        <Stack component="form" spacing={3} onSubmit={handleSubmit(submitForm)}>
          <Stack spacing={1}>
            ...
            <Typography color="text.secondary">
              Complete email verification and recruiter contact details to start
              submitting JNFs.
            </Typography>
          </Stack>
              ...
              <Grid size={{ xs: 12, md: 6 }}>
                <Card variant="outlined">
                  <CardContent>
                    ...
                    <Typography
                      color={
                        countdown.expired ? "error.main" : "text.secondary"
                      }
                    >
