...
class InfController extends Controller
{
    ...
    {
        $this->authorizeAccess($request, $inf);
        abort_unless(in_array($inf->status, ['draft', 'submitted', 'open_edit'], true), 409, 'Only draft, submitted, or open-edit INFs can be updated.');
    ...
    {
        $this->authorizeAccess($request, $inf);
        abort_unless(in_array($inf->status, ['draft', 'submitted', 'open_edit'], true), 409, 'This INF is not editable.');
    ...
    public function duplicate(Request $request, Inf $inf): JsonResponse
    {
        ...
        $clone = Inf::query()->create([
            'company_id' => $inf->company_id,
            'title' => ($inf->title ?? 'INF').' (Copy)',
            'description' => $inf->description,
            'status' => 'draft',
        ]);

        if ($inf->internshipProfile !== null) {
            $clone->internshipProfile()->create(collect($inf->internshipProfile->toArray())
                ->except(['inf_id', 'created_at'])
                ->all());
        }

        return response()->json(['data' => $clone->fresh(['company', 'internshipProfile'])], 201);
    }
