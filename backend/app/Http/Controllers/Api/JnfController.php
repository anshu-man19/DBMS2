...
class JnfController extends Controller
{
    ...
    {
        $this->authorizeAccess($request, $jnf);
        abort_unless(in_array($jnf->status, ['draft', 'submitted', 'open_edit'], true), 409, 'Only draft, submitted, or open-edit JNFs can be updated.');
    ...
    {
        $this->authorizeAccess($request, $jnf);
        abort_unless(in_array($jnf->status, ['draft', 'submitted', 'open_edit'], true), 409, 'This JNF is not editable.');
    ...
    public function duplicate(Request $request, Jnf $jnf): JsonResponse
    {
        ...
        $clone = Jnf::query()->create([
            'company_id' => $jnf->company_id,
            'title' => ($jnf->title ?? 'JNF').' (Copy)',
            'description' => $jnf->description,
            'status' => 'draft',
        ]);

        if ($jnf->jobProfile !== null) {
            $clone->jobProfile()->create(collect($jnf->jobProfile->toArray())
                ->except(['jnf_id', 'created_at'])
                ->all());
        }

        return response()->json(['data' => $clone->fresh(['company', 'jobProfile'])], 201);
    }
